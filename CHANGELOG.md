...
### Added
- **CapCut-style video editor** — timeline with draggable clips, preview scrubber, trim handles, effects (fade, speed, filters, text), and auto-save to `project.editorData`
- **Shorts / Reels panel** — YouTube Short, Instagram Reel, TikTok presets with 9:16 browser export
- **Browser ffmpeg.wasm render** — client-side timeline export + offline bundle (`timeline.json` + `render.sh`)
- **Storyboard import** — build timeline from shots + assets in one click
- `/projects/[id]/editor` route and Editor nav tab
- `scripts/seed-demo-editor.mjs` — admin user + "Neon City Chronicles" demo (~2 min, 12 CC0 clips); `pnpm run db:seed-demo`
- `docs/EDITOR.md` — architecture and user guide
...
[0.3.0]: https://github.com/krnbwj/ai-video-story-studio/compare/v0.2.1...v0.3.0
